...
✅ **DECIDED (2026-08-25): Option B (container rootfs first).** So
...
> This is maintainer decision and touches host machinery (running builds/containers,
...
| P2 | `--headless/--gui/--keep/--no-kvm/--memory/--cpus/--gdb` | ✅ **done (2026-08-28)** — full flag set for the `qemu` runtime: KVM auto-detect w/ TCG fallback (+`--no-kvm`), `-m`/`-smp`, `-nographic` vs display, `-gdb tcp::1234 -S`, persistent serial log under `.logs/qemu/` (gitignored), effective-params printout. **`--disk` immutability (§4.1): a disposable writable QCOW2 overlay** (`qemu-img create -b <base> -F raw`) is booted so the canonical store image is NEVER modified — rm'd on exit, retained+printed with `--keep` (verified). `--ssh-port/--share-vyb` still recognized + rejected (need in-guest SSH / guest virtiofs) | `--runtime qemu --test` and `--disk … --test` both PASS (READY) on TCG |
...
| P5 | Wire to a real VybOS boot target; VybOS-specific smoke checks | ⏳ **persistent root/disk image LANDED + `--disk` boots it (B5) + generation switch LANDED** (`build-gensys.vyb`+`gensys.vyb`: N gens + `current` pointer; init prints ACTIVE_GENERATION; two variants each boot their own gen — verified). Remaining: bootloader, VybOS userspace, atomic `vyb system switch` command (once `rename`/`symlink` RFE lands) | `--runtime qemu --disk store/<ca>/… --test` PASS on both gen variants |
...
## 9. Open questions for maintainers (before/at P0)
